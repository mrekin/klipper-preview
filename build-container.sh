...
# Docker image build script for klipper-print-share with versioning
# Author: Claude Code
...
# Variables
IMAGE_NAME="klipper-print-share"
...
get_package_version() {
    if [ -f "$PROJECT_DIR/package.json" ]; then
        local pkg_version=$(grep '"version"' "$PROJECT_DIR/package.json" | head -1 | sed 's/.*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')
...
main() {
    ...
    echo -e "${BLUE}================================${NC}"
    echo -e "${BLUE}Klipper Print Share - Docker Build${NC}"
    echo -e "${BLUE}================================${NC}"
    echo
